            ...
            @forelse ($actualities as $item)
                <div class="w-full flex-shrink-0 h-72 bg-white dark:bg-slate-900 flex flex-col">
                    <div class="relative h-32 w-full overflow-hidden">
                        <img src="{{ $item->image ? Storage::disk('s3')->url($item->image) : asset('img/actuality-default.jpg') }}" alt="{{ $item->title }}"
                    ...
                    <div class="p-4 flex-grow flex flex-col">
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-400 mt-2 flex-grow">{{ strip_tags(Str::limit($item->content, 250)) }}</p>
