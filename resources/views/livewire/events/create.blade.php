<div>
    <form wire:submit.prevent="createEvent" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" id="start_date" wire:model.defer="start_date"
                    class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                @error('start_date') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>

            <div>
                <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">Date de fin <span class="text-gray-400 text-xs">(optionnel)</span></label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label for="start_time" class="block text-sm font-medium text-gray-700 mb-1">Heure de début <span class="text-gray-400 text-xs">(optionnel)</span></label>
            ...
            <div>
                <label for="end_time" class="block text-sm font-medium text-gray-700 mb-1">Heure de fin <span class="text-gray-400 text-xs">(optionnel)</span></label>
        ...
        <div>
            ...
            @if ($image)
                <div class="mt-2">
                    <img alt="Image de l'événement" src="{{ $image->temporaryUrl() }}" class="h-32 w-auto object-cover rounded-lg">
                </div>
            @endif
        </div>
