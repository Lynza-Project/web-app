...
use App\Models\User;
use App\Policies\ActualityPolicy;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

class ActualityPolicyTest extends TestCase
{
    use RefreshDatabase;

    private ActualityPolicy $policy;
    ...
    protected function setUp(): void
    {
        ...
        $this->policy = new ActualityPolicy();
        ...
        $this->actuality = Actuality::factory()->create([
            'organization_id' => $this->organization1->id,
        ]);
    }
    ...
    {
        $this->assertTrue($this->policy->view($this->user, $this->actuality));
    }
    ...
    public function test_view_returns_false_for_user_in_different_organization()
    {
        ...
        $this->assertFalse($this->policy->view($userFromOtherOrg, $this->actuality));
    }
    ...
    {
        $this->assertTrue($this->policy->update($this->superAdmin, $this->actuality));
    }

    public function test_update_returns_true_for_admin_in_same_organization()
    {
        $this->assertTrue($this->policy->update($this->admin, $this->actuality));
    }
    ...
    public function test_update_returns_false_for_admin_in_different_organization()
    {
        ...
        $this->assertFalse($this->policy->update($adminFromOtherOrg, $this->actuality));
    }

    public function test_update_returns_false_for_regular_user()
    {
        $this->assertFalse($this->policy->update($this->user, $this->actuality));
    }

    public function test_delete_returns_true_for_super_admin()
    {
        $this->assertTrue($this->policy->delete($this->superAdmin, $this->actuality));
    }

    public function test_delete_returns_true_for_admin_in_same_organization()
    {
        $this->assertTrue($this->policy->delete($this->admin, $this->actuality));
    }
    ...
    public function test_delete_returns_false_for_admin_in_different_organization()
    {
        ...
        $this->assertFalse($this->policy->delete($adminFromOtherOrg, $this->actuality));
    }

    public function test_delete_returns_false_for_regular_user()
    {
        $this->assertFalse($this->policy->delete($this->user, $this->actuality));
    }
